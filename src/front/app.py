...
VAULT_PATH = BASE_PATH / "vault_container"
...
def encrypt():
    ...
    try:
        ...
        return send_file(io.BytesIO(data), as_attachment=True,
                        download_name=f"{file.filename}_vault.zip", mimetype='application/zip')
...
def decrypt():
    ...
    filename    = re.sub(r"\.zip$","",file.filename, flags=re.IGNORECASE)
    uid         = secrets.token_hex(4)
    ...
    vault_dir   = os.path.join(VAULT_PATH, 
                               filename if filename.endswith("_vault") else f"{filename}_vault")
    header_path = os.path.join(vault_dir, "header.json")

    with open(header_path, "r", encoding="utf-8") as f:
        header = json.load(f)

    filename = header.get("file_name", filename)
    suffix = Path(filename).suffix or ""
    out_path = os.path.join(VAULT_PATH, f"out_{uid}{suffix}")
...
def download_zip(container_name):
    ...
    return send_file(io.BytesIO(data),as_attachment=True,
                     download_name=f"{container_name}.zip", mimetype='application/zip')
